...
val_split = int((8 / 12) * len(test_X))
val_X, val_y = test_X[:val_split], test_y[:val_split]
test_X, test_y = test_X[val_split:], test_y[val_split:]

model = setup_lstm_model(train_X,100)
# fit network
history = model.fit(train_X, train_y,  epochs=200, batch_size=14, validation_data=(val_X, val_y), verbose=2)
...
plt.savefig("../Images/Power_LSTM_train_loss.png")
plt.close()
...
fig_verify.savefig("../Images/Power_LSTM_train_plot.png")
